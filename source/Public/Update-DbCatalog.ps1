function Update-DbCatalog {
    ...
    foreach ($t in $tables) {
        $tn = $t.name
        $rowcount = (Invoke-DbQuery -Database $Database -Query "SELECT COUNT(*) AS c FROM $(Quote-Ident $tn)")[0].c
...
"@ -SqlParameters @{ t = $tn; s = $SourceCsvPath; h = $csvHash; c = $rowcount; j = $null } -NonQuery | Out-Null
        ...
        $cols = Invoke-DbQuery -Database $Database -Query "PRAGMA table_info($(Quote-Ident $tn))"
        foreach ($c in $cols) {
            Invoke-DbQuery -Database $Database -Query @"
...
"@ -SqlParameters @{ t = $tn; c = $c.name; dt = $c.type; n = [int](-not [bool]$c.notnull); pk = [int]$c.pk; ex = $null } -NonQuery | Out-Null
        ...
        $fks = Invoke-DbQuery -Database $Database -Query "PRAGMA foreign_key_list($(Quote-Ident $tn))"
        foreach ($fk in $fks) {
            Invoke-DbQuery -Database $Database -Query @"
